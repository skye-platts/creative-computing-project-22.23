...
fftProcessorMultiThread::fftProcessorMultiThread()
{
...
//--------------------------------------------------
void fftProcessorMultiThread::setup(unsigned int win, unsigned int hop, unsigned int circBuffSize)
{
    ...
    outputBuffer.resize(circularBufferSize);
    
}

//--------------------------------------------------
void fftProcessorMultiThread::setPitchShift(float factor)
{
    pitchShift = factor;
}

//--------------------------------------------------
float fftProcessorMultiThread::wrapPhase(float phaseIn)
{
    if (phaseIn >= 0) {
...
void fftProcessorMultiThread::prepFFT(std::vector<float> &inBuffer, unsigned int inPointer, std::vector<float> &outBuffer, unsigned int outWritePointer) {
    
    std::cout << "PREPPED" << std::endl;
    
    inputBuffer.assign(inBuffer.begin(), inBuffer.end());
    inputReadPointer = inPointer;
    outputWritePointer = outWritePointer;
}


//--------------------------------------------------
void fftProcessorMultiThread::threadedFunction(){
    ...
    while (isThreadRunning()) {
        ...
        for (int n = 0; n < fftSize; n++) {
            ...
            int circularBufferIndex = (inputReadPointer + n - fftSize + circularBufferSize) % circularBufferSize;
            unwrappedBuffer[n] = inputBuffer[circularBufferIndex] * windowFunction[n];
        }
        for (int n = 0; n < fftSize; n++) {
        ...
        for (int n = 0; n < fftSize; n++) {
            int circularBufferIndex = (outputWritePointer + n - fftSize + circularBufferSize) % circularBufferSize;
            outputBuffer[circularBufferIndex] += fftwSynthesisReal[n] * windowFunction[n];
        }
        
        // NOW IN AUDIO LOOP - after thread has been executed
        // Increment circular buffer write pointer by the hop size using the reference
        ...
        std::cout << "Thread executed : " << outputWritePointer << std::endl;
        stopThread();
